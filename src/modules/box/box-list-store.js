...
import boxListExporterFields from '@/modules/box/box-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (!boxListExporterFields || !boxListExporterFields.length) {
          throw new Error('boxListExporterFields is required');
        }
