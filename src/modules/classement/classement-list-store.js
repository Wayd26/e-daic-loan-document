...
import classementListExporterFields from '@/modules/classement/classement-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (!classementListExporterFields || !classementListExporterFields.length) {
          throw new Error('classementListExporterFields is required');
        }
        ...
        const response = await ClassementService.list(
          filter,
          getters.orderBy,
          null,
          null,
