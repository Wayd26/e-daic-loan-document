...
import folderListExporterFields from '@/modules/folder/folder-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (!folderListExporterFields || !folderListExporterFields.length) {
          throw new Error('folderListExporterFields is required');
        }
