...
import divisionListExporterFields from '@/modules/division/division-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (!divisionListExporterFields || !divisionListExporterFields.length) {
          throw new Error('divisionListExporterFields is required');
        }
        ...
        const response = await DivisionService.list(
          filter,
          getters.orderBy,
          null,
          null,
